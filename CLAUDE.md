...
/Desktop/Gallery/
  ...
  mitm.html     ← StreamSaver (self-hosted) — usado por Export para streaming a disco
  sw.js         ← StreamSaver service worker (self-hosted)
...
- [x] Export: botón "Export" en el header descarga un backup ZIP de toda la librería. Usa JSZip (CDN esm.sh). Estructura: una carpeta por tag (subtags `a/b` → carpetas anidadas) con copia de cada imagen en cada tag que tenga, carpeta `_uncategorized` para las sin tags, y `metadata.json` en la raíz con tags/notas/flows/descripciones para restaurar. Overlay de progreso con cancelar. Pagina el storage list (>500 items). Descarga vía fetch del publicUrl con timeout 45s + 2 reintentos, en paralelo (5 a la vez) para no colgarse en un archivo. Reporta total/descargadas/fallidas en overlay + _EXPORT-REPORT.txt + metadata.json. **Streaming a disco** vía `client-zip` (`makeZip`) + **StreamSaver** (mitm.html + sw.js self-hosted) → escribe el ZIP al disco a través de un service worker sin acumularlo en RAM. Imprescindible con ~900+ imágenes y compatible con Brave (que bloquea `showSaveFilePicker`/File System Access API y service workers de terceros, por eso el mitm/sw van self-hosted). Fallback sin service worker: `downloadZip().blob()` en memoria. Descarga con prefetch de 5 en orden. NOTA: ya NO se usa JSZip ni la File System Access API
